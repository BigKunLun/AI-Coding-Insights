...
- **隐私**：**边界要说准，别复述「原文永不出本机」——那是假的**。本项目无服务器、不收集数据，报告是本地 HTML；但 extractor 是 LLM 子代理，`batch-NN.json` 里的会话原文（含 `cwd` / `file_path` / 完整 `turns[].text`）**确实会经用户自己那家 agent 的模型 API**。可准确宣称的是三条：①本项目自身零网络出口（规则层纯本地）②**业务语义不进画像/报告/快照**③原文只到第一轮 extractor，**五专家被禁止回读 batch 原文**，只读脱敏后的 obs。对外文案（README / 投稿稿）必须同时讲清「没有服务器」与「会走你自己 agent 的模型」，并点明真实例外：会话若来自第三方模型，在 CC 里分析就是跨厂商新增暴露。承重机制（改动即可能捅破隐私网）：归属边界单点在 `Config.discovery_rules`；`_aggregate.json` 与跨次快照主动剥离含项目名的 `project_breakdown`；批次出规则层前过 `redact` 密钥网（**只是密钥网，不剥业务语义**）；extractor → obs 这道抽象层是业务语义的真正闸门；文件路径仅在 `git_outcome` 本机内做交集匹配、只出计数，路径与 diff 从不进模型。
