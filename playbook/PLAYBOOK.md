...
!`<ACI> scan --source <SOURCE> <PLUGIN_ROOT_OPT> --emit-batches "${HOME}/.ai-coding-insights/run"`
...
```
<ACI> verify-obs --batches <BATCHES_DIR> --obs-glob '<BATCHES_DIR>/obs-*.json'
```
...
```
<ACI> render-profile --plugin-root <PLUGIN_ROOT> --profile <BATCHES_DIR>/profile.json --metrics <BATCHES_DIR>/_aggregate.json --window <BATCHES_DIR>/_window.json --obs-glob '<BATCHES_DIR>/obs-*.json' --session-count <N> --project <项目1> --project <项目2> --run-started <RUN_STARTED> --run-agents <AGENT_N>
```
