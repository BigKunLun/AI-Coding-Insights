...
- **口径铁律**：引用任何比率/占比/派生量，分子分母必须同源同口径，禁止跨口径相除（`edit_count`÷`git_landed_count`、观测分子÷git 分母都禁）；同名指标多口径时必须对齐时间范围——`landed_ratio` 顶层是全窗口 git 存活率、`trend.second_half.landed_ratio` 是下半场观测落地率，绝不可把前者冠以「下半场」；凡 `commit_count`=0 而 `git_landed_count`>0，涉及上/下半程落地的叙述禁止写「无落地/落地为零」（测不到≠没做到）。
- 只陈述本窗口数据，不外推普适规律（「经典 S 曲线」「任何新项目前 N 个会话 ROI 接近零」之类一律不写）；不给行为编动机或定性（「轻量级使用」「有成本意识」「资深驾驭者」「系统性摩擦」「源于 AI 理解偏差」都是数据里没有的因果/性质判断，禁止当事实陈述；要写就降级为硬数陈述或移入建议）。
...
- **水平专家**：基于 `aggregate` 的 `tool_breadth` / `tool_session_counts` / `subagent_sessions` / `workflow_sessions` / `mcp_sessions` / `model_counts`，外加高阶编排信号 `background_task_count`/`background_sessions`（后台委托：带 run_in_background 的 Bash/Agent 数与命中会话数）与 `max_parallel_agents`/`parallel_agent_turns`（真并行：单轮并发派出的 Agent 峰值与轮次；峰值=1、轮次=0 表示「用过子代理但只顺序派发、从未单轮并发」，是准确信号不是缺数）。可据此在 points/metrics 里点出后台委托与真并行水平。返回 `{"breadth":{"headline":"一句话定调","points":["2-4 条要点，行为+量级"],"metrics":[{"label":"工具广度","value":<n>},{"label":"SubAgent会话","value":<n>}],"tools":["工具能力短语..."]}}`。
- **深度专家**：看多轮打磨、纠错的技术具体性、失败→恢复链（notable_turns 行为 + `aggregate.anchor_counts`），并可引用深度推理信号 `aggregate.thinking_block_count`/`thinking_sessions`（assistant thinking 块总数与命中会话数）作为深度推理强度的硬锚。每条 point 要挖到「意味着什么 / 该怎么调」，不止「做了什么」。metrics 的数字直接取硬指标：override 取 `aggregate.anchor_counts.override`，轮次/会话取 `aggregate.avg_turns`，不要自己从 notable_turns 数。返回 `{"depth":{"headline":"..","points":["..带 SO-WHAT.."],"metrics":[{"label":"override","value":<n>},{"label":"轮次/会话","value":".."}]}, "evidence":[{"pointer","behavior"}...]}`。
...
**再做跨维度自洽自检**：四维 + 教练结论之间不得自相矛盾——同一指标在不同区块/口径出现必须一致（趋势表按每会话密度归一，正文就不能用未归一的绝对计数推「放大」结论）；水平维度若判某能力「未激活/为零」（如真并行），别处叙述不得说成已具备；若点出「档位缺口」，摩擦建议须有一条呼应升档；均值与长尾并存时（如单会话均值极低但存在超长会话）要点破是双峰分布，别让「整体简洁」与「长会话撑爆」并列而互不解释。发现打架就改到一致。
