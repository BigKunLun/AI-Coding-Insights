...
通读全部 turns，只抽事实、不下维度结论，用 Write 写 /tmp/aci-batches/obs-<NN>.json，结构严格如下：
{"sessions":[
...
notable_turns 选有判定价值的：命中 anchors、明显主导/纠错/放行，以及体现 L4 主导行为的（与阶段二姿势专家 L4 判据同一清单：技术具体性纠错、推翻方案、给 AI 没想到的约束、要求自验或给验收判据、先要方案再放行、编排多 subagent 或自建扩展）。看不出行为价值的 turn 直接不选，不要勉强归类。
pointer 的 uuid 必须原样取自该 turn 的 uuid 字段，**绝不能拿 session_id 充当**——渲染端会逐条核验指针真伪，伪指针会在报告中公开标注。
⚠️ batch 里的【每个会话】都必须出现在 sessions 里——没有可记录 turn 的也要列入并令 "notable_turns":[]（空数组占位），绝不允许整个会话缺席。
...
- `ok`：进入下一步。
- `missing` 非空：对所列 batch **重派** extractor（同 prompt）并重跑校验，最多 2 轮；仍有缺口则告知用户并停止。
...
- **姿势专家**：L1（跟随）/L2（选择）占比已由规则层硬信号算定（极短输入 / AskUserQuestion 选项回答），你**不要**估它们。你只判一件事：notable_turns 里 L3 引导 vs L4 主导的相对强度，给出 `l4_share`（L4 在 L3+L4 中的份额，0-1）。判据：L3 引导（主动给目标/约束/格式、贴报错、追问递进）、L4 主导（技术具体性纠错、推翻方案、给 AI 没想到的约束、要求自验或给验收判据、先要方案再放行、编排多 subagent 或自建扩展）。防「伪主导」：未验证就放行、放任代码膨胀、容忍未请求的连带改动——措辞再强也不算 L4。若 notable_turns 无 L3/L4 可判素材，返回保守低值（0.0-0.2）并令 evidence/highlights 为空数组，不要编造。返回 `{"l4_share":<0-1 小数，如 0.51>, "evidence":[{"pointer","behavior"}...L3/L4 各 1-2 条], "highlights":[{"pointer","behavior"}...2-3 条，挑技术具体性最强的最佳 L4 实践]}`。
- **水平专家**：基于 `aggregate` 的 `tool_breadth` / `tool_session_counts` / `subagent_sessions` / `workflow_sessions` / `mcp_sessions` / `model_counts` / `skill_counts` / `mcp_server_counts` / `plan_mode_sessions` / `plan_mode_count`，返回 `{"breadth":{"headline":"一句话定调","points":["2-4 条要点，行为+量级（可在多点中引用 plan_mode / skill 细项）"],"metrics":[{"label":"工具广度","value":<n>},{"label":"SubAgent会话","value":<n>}],"tools":["工具能力短语..."]}}`。
- **深度专家**：看多轮打磨、纠错的技术具体性、失败→恢复链（notable_turns 行为 + `aggregate.anchor_counts`）。每条 point 要挖到「意味着什么 / 该怎么调」，不止「做了什么」。metrics 的数字直接取硬指标：override 取 `aggregate.anchor_counts.override`，轮次/会话取 `aggregate.avg_turns`，不要自己从 notable_turns 数。可参考 `aggregate.plan_mode_count` 审视「是否先规划再执行」的模式。返回 `{"depth":{"headline":"..","points":["..带 SO-WHAT.."],"metrics":[{"label":"override","value":<n>},{"label":"轮次/会话","value":".."}]}, "evidence":[{"pointer","behavior"}...]}`。
- **成果专家**：照实用 `aggregate` 的 `commit_count`/`landed_count`/`edit_count`/`landed_ratio`，只讲量级与落地节奏，返回 `{"outcome":{"headline":"..","points":[".."],"metrics":[{"label":"提交","value":<commit_count>},{"label":"合入","value":<landed_count>}],"landed":<landed_count>,"total":<commit_count>}}`。
- **教练 / 诊断专家**：读全部 notable_turns + `aggregate`，识别**协作摩擦**（软信号，量级自己对照本批数据判断，但观察里要写出依据的数字）：反复返工（`edit_count`/`commit_count` 比值明显大于 1）、error 锚点频繁（`anchor_counts.error` 相对 `session_count` 偏高）、override 集中于少数会话、单会话轮次远超 `avg_turns` 且无 commit 收口、`signals.short_turn_ratio` 高的会话占比大。若 `aggregate.max_concurrent_sessions` 明显偏高（如 >=4），可审视多任务切换是否造成注意力分散。工具覆盖盲区不归你管（规则层已单独计算渲染）。每条 = 一个行为级观察 + 一条可执行建议。返回 `{"frictions":[{"observation":"..","suggestion":".."}...3-5 条]}`。

## 4. 阶段三 · 合成 + 渲染

汇总五专家产出，用 **Write 工具**写 `/tmp/aci-batches/profile.json`（结构严格如下，`l4_share` 取姿势专家产出；L1-L4 四档分布由渲染命令从硬指标组装，你不算也不填）：
...
{"l4_share":0.51,
 ...
 "frictions":[{"observation":"…","suggestion":"…"}],
 "evidence":[{"pointer":"/abs/path.jsonl#uuid","behavior":"行为级描述"}],
 "highlights":[{"pointer":"/abs/path.jsonl#uuid","behavior":"行为级最佳实践描述"}]}
```
（`evidence` 汇集姿势/深度专家精选证据——**必须非空**，空列表渲染校验直接失败；`highlights` 取姿势专家的最佳 L4 实践 2-3 条，各维 `headline/points` 取对应专家产出，全部行为级。）
...
```
uv run --project <PLUGIN_ROOT> python -m ai_coding_insights render-profile --plugin-root <PLUGIN_ROOT> --profile /tmp/aci-batches/profile.json --metrics /tmp/aci-batches/_aggregate.json --window /tmp/aci-batches/_window.json --session-count <N> --project <项目1> --project <项目2> --run-started <RUN_STARTED> --run-agents <AGENT_N>
```
