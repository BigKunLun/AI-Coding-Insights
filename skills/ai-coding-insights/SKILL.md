...
- **教练 / 诊断专家**：读全部 notable_turns + `aggregate`，识别**协作摩擦**（软信号，量级自己对照本批数据判断，但观察里要写出依据的数字）：反复返工（`edit_count`/`commit_count` 比值明显大于 1）、error 锚点频繁（`anchor_counts.error` 相对 `session_count` 偏高）、override 集中于少数会话、单会话轮次远超 `avg_turns` 且无 commit 收口、`signals.short_turn_ratio` 高的会话占比大。工具覆盖盲区不归你管（规则层已单独计算渲染）。每条 = 一个行为级观察 + 一条可执行建议。返回 `{"frictions":[{"observation":"..","suggestion":".."}...3-5 条]}`。
