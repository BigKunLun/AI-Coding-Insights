...
没有服务器、不要 API key，报告只落你本机，结论留给你自己。
...
![Local-first](https://img.shields.io/badge/privacy-无服务器%20·%20报告只在本机-orange)
![Version](https://img.shields.io/badge/version-1.0.0--alpha.1-lightgrey)

</div>

只读你本机的 AI 编码会话记录，给你一份**协作画像 + 摩擦建议**的本地 HTML 报告。
**报告与业务语义不出本机**，本项目没有服务器、不收集任何数据——语义分析那一步走的是
**你自己 agent 的模型**（详见 [「它凭什么可信」](#它凭什么可信)里的边界说明）。
机器只给分析与证据，结论与判断在你自己。
...
**边界（这条得先讲明白）**：本项目**没有服务器、不收集任何数据**，报告是纯本地 HTML。
但语义分析要靠模型，而模型跑在**你自己的 agent** 里——所以**会话片段会经你自己那家
agent 的模型 API**，和你平时用它写代码走的是同一条路、同一份订阅、同一份隐私协议。
说白了：分析你 Claude Code 的会话，等于把**已经给过 Anthropic 的文本再给它一次，不新增接收方**。
想一个字都不进模型，跑[「只跑规则层」](#不想花-token只跑规则层)那条命令——那条路径**没有任何模型调用**，只出硬指标。

> ⚠️ **一个真实的例外**：若你的会话本身来自别家模型（比如在 Claude Code 里挂了第三方
> 模型跑的会话），那么在 Claude Code 里分析它，等于把原本只给过 A 家的文本送到 B 家。
> 这是**跨厂商的新增暴露**，工具替你挡不掉，请自己判断。

**机制（不是口头保证，是落在代码上的）**：

- **报告产物只含行为与量级**：进入画像 / 证据 / 建议的所有自由文本只描述**行为模式与量级**，绝不含客户 / 功能 / 产品 / 架构等业务内容；脱敏校验不过直接失败，不出报告。
- **两段式脱敏**：模型分两轮跑——第一轮 extractor 把原文压成**行为级事实**，之后的五个分析专家**只读脱敏结果、被明令禁止回读原文**。业务语义在第一道墙就被剥掉，不会流进画像。
- **业务标识不进中间产物**：含项目名的数据不进跨次快照、不进 `_aggregate.json`；报告里项目只以序号出现。
- **密钥网兜底**：进入模型的文本在出规则层前就地脱敏，覆盖私钥 / JWT / 各厂商 token / 连接串口令，取向宁可过度脱敏。
- **文件路径只在本机内求交**：git 落地判定要用到「提交改动文件 ∩ 会话编辑文件」，这个交集只在你机器上算，**路径与 diff 从不进入模型，只出计数**。
- **证据可信**：每条证据指针逐条回看核验，模型编造的路径会在报告里公开标注「指针未命中」。
...
硬指标全部由规则层确定性计算，**不经过任何 LLM**。只要不进语义分析那一步，
就一分 token 不花，**也没有任何一个字节离开你的机器**（进程内只读会话文件、跑 git、写 JSON）：
